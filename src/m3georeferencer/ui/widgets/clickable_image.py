...
# Local
from m3georeferencer.validators.image_validators import (
...
class ClickableImage(QWidget):
    ...
    def image_data(self, value: np.ndarray) -> None:
        ...
        imview_config = _validate_image_data(value)

        # In the case of an "empty" (i.e. size = 1) third dimension
        if value.ndim == 3 and imview_config["desc"] == "flat":
            self._image_data = value[:, :, 0]
        else:
        ...
        rect.setPen(pg.mkPen("r", width=4))
        rect.setFlag(QGraphicsItem.GraphicsItemFlag.ItemIgnoresTransformations)
        rect.setZValue(1000)
        self._vbox.addItem(rect)

        self._border_item = rect
        self._border_item.setVisible(False)
    ...
    def toggle_border(self) -> None:
        # self._border_item.setVisible(not self._border_item.isVisible())
        gview = self._vbox.scene().views()[0]
        self._border_visible = not self._border_visible
        if self._border_visible:
            gview.setStyleSheet("border: 3px solid red")
        else:
            gview.setStyleSheet("")
